...
public final class LynxShipNotificationPermissionActivity extends Activity {
    ...
    protected void onCreate(Bundle state) {
        ...
        if (getIntent().getStringExtra(LynxShipNotificationsStore.EXTRA_REQUEST_ID) == null) {
            finish();
            return;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            LynxShipNotificationsStore.sendPermissionResult(
                getApplicationContext(),
                getIntent().getStringExtra(LynxShipNotificationsStore.EXTRA_REQUEST_ID),
                true);
            finish();
        }
    }
